...
func execute(session: GameSession, command: GameCommand) -> CommandResult:
	...
	if command is PlaceBuildingCommand:
		...
		return _complete(session, command, session.execute_place_building(place_command))
	if command is UpgradeBuildingCommand:
		...
		return _complete(session, command, session.execute_upgrade_building(upgrade_building_command))
	if command is UpgradeRaftCommand:
		var upgrade_raft_command: UpgradeRaftCommand = command as UpgradeRaftCommand
		return _complete(session, command, session.execute_upgrade_raft(upgrade_raft_command))
	if command is GatherResourcesCommand:
		var gather_command: GatherResourcesCommand = command as GatherResourcesCommand
		return _complete(session, command, session.execute_gather_resources(gather_command))
	if command is UseFoodCommand:
		var use_food_command: UseFoodCommand = command as UseFoodCommand
		return _complete(session, command, session.execute_use_food(use_food_command))
	...
	if command is BattleActionCommand:
		...
		return _complete(session, command, session.execute_battle_action(battle_action_command))
	if command is ReturnFromBattleCommand:
		var return_battle_result: CommandResult = _complete(session, command, session.execute_return_from_battle(command as ReturnFromBattleCommand))
...
func _complete(session: GameSession, command: GameCommand, result: CommandResult) -> CommandResult:
	if result != null and result.succeeded:
		session.evaluate_quests()
	elif result != null:
